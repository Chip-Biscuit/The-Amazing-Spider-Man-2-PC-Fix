...
//Chips The amazing spiderman 2 patches added.
...
//=======================================================================================================================================================================================
// chip - toggle where user chooses input with message box confirmation - not used in amazing spiderman 2 but keep incase needed in the future
...
//=======================================================================================================================================================================================
// chip - 2: fov - - not used in amazing spiderman 2 but keep incase needed in the future
...
//=======================================================================================================================================================================================
// chip - 3: fov 2 - not used in amazing spiderman 2 but keep incase needed in the future
...
bool GetWindowModeSetting()
{
    ...
    LONG res = RegOpenKeyEx(HKEY_CURRENT_USER,
        _T("SOFTWARE\\Activision\\The Amazing Spider-Man 2 (TM)\\Settings\\Display"),
        0, KEY_READ, &hKey);
...
bool GetResolutionSettings(int& resolutionWidth, int& resolutionHeight)
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\Activision\\The Amazing Spider-Man 2 (TM)\\Settings\\Display"), 0, KEY_READ, &hKey) == ERROR_SUCCESS)
    {
...
void SetResolutionSettings(int resolutionWidth, int resolutionHeight)
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\Activision\\The Amazing Spider-Man 2 (TM)\\Settings\\Display"), 0, KEY_WRITE, &hKey) == ERROR_SUCCESS)
    {
...
// HKEY_CURRENT_USER\Software\Activision\The Amazing Spider-Man 2 (TM)\Settings\Display\AspectRatio
...
//=======================================================================================================================================================================================
// chip - vsync preset (hardcoded)
//=======================================================================================================================================================================================

// Read VSync (returns value, or 0 on error/missing)
DWORD GetVSyncSetting()
{
    HKEY hKey = NULL;
    DWORD dwValue = 0;
    DWORD dwSize = sizeof(dwValue);
    ...
    {
        if (RegQueryValueEx(hKey, _T("VSync"), NULL, &dwType, (LPBYTE)&dwValue, &dwSize) == ERROR_SUCCESS)
        {
            if (dwType == REG_DWORD)
            {
                RegCloseKey(hKey);
...
// Set VSync to decimal 0 under:
// HKEY_CURRENT_USER\Software\Activision\The Amazing Spider-Man 2 (TM)\Settings\Display\VSync
// Returns true on success.
bool SetVSyncTo0()
{
    HKEY hKey = NULL;
    DWORD dwDisposition = 0;
    DWORD dwValue = 0; // hard-coded value
    ...
    res = RegSetValueEx(hKey, _T("VSync"), 0, REG_DWORD, (const BYTE*)&dwValue, sizeof(dwValue));
    RegCloseKey(hKey);
    return (res == ERROR_SUCCESS);
}

// Main function to be called when the DLL is injected (keeps same signature as your other toggles)
// This simply sets VSync = 0 (no INI checks)
void ToggleVSync(HMODULE /*hModule*/)
{
    SetVSyncTo0();
}
